...
extension UIKnob {
    ...
    func metallLayer(size: CGSize) -> CALayer {
        let frame             = CGRect(origin: .zero, size: size)
        let circlesNum        = 6
        let colorWhite0       = CGFloat(0.7)
        let colorWhite1       = colorWhite0 + 0.007
        let layer             = CAShapeLayer()
        layer.fillColor       = UIColor(white: colorWhite0, alpha: 1).cgColor
        layer.backgroundColor = UIColor(white: colorWhite1, alpha: 1).cgColor
        layer.frame           = frame
        layer.fillRule        = .evenOdd

        let path = CGMutablePath()
        for circle in 0..<circlesNum {
            let ellipseW = frame.width*CGFloat(circle)/CGFloat(circlesNum)
            let ellipseH = frame.height*CGFloat(circle)/CGFloat(circlesNum)
            path.addEllipse(in:
